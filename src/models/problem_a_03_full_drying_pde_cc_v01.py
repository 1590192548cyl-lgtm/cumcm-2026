...
class RadialDryingSolver:
    ...
    def solve_sampled(
        ...
        while chunk_start < end_time_s:
            ...
            atol = np.concatenate(
                [
                    np.full(self.n_nodes, 1.0e-8),
                    np.full(self.n_nodes, 1.0e-9),
                ]
            )
            solution = solve_ivp(
                self.rhs,
            ...
            for time_s, sampled_state in zip(
                ...
                centre_moisture = sampled_state[self.n_nodes]
                if (
                    stop_at_dry
                    and drying_time_s is None
                    and centre_moisture <= DRYING_THRESHOLD
                ):
                    previous_time = collected_time[-2]
                    previous_centre = collected_moisture[-2][0]
                    fraction = (previous_centre - DRYING_THRESHOLD) / max(
                        previous_centre - centre_moisture, 1.0e-15
                    )
                    drying_time_s = previous_time + fraction * (
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--radius-input",
        type=Path,
        default=Path("data/raw/problem_a_raw_radius_history_v01.xlsx"),
    )
...
def main() -> None:
    ...
    diagnostics = {
        ...
        "question_3": {
            ...
            "drying_time_s_interpolated": float(q3["drying_time_s"]),
            "drying_time_h_interpolated": float(q3["drying_time_s"] / 3600.0),
            "coarse_drying_time_h_interpolated": float(
                q3_coarse["drying_time_s"] / 3600.0
            ),
            "drying_time_abs_difference_h": float(
                abs(q3["drying_time_s"] - q3_coarse["drying_time_s"]) / 3600.0
            ),
        ...
        "question_4": {
            ...
            "drying_time_s_interpolated": float(q4["drying_time_s"]),
            "drying_time_h_interpolated": float(q4["drying_time_s"] / 3600.0),
            "coarse_drying_time_h_interpolated": float(
                q4_coarse["drying_time_s"] / 3600.0
            ),
            "drying_time_abs_difference_h": float(
                abs(q4["drying_time_s"] - q4_coarse["drying_time_s"]) / 3600.0
            ),
            "six_hour_max_abs_moisture_difference_kg_per_kg": float(
