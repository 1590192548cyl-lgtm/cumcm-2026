...
async function addSheet(workbook, spec) {
  ...
  if (rows[0][0] !== spec.firstTime || rows.at(-1)[0] !== spec.lastTime) {
  ...
  sheet.getRange(`A2:${lastColumn}${spec.expectedRows + 1}`).values = rows;
  styleResultSheet(
    sheet,
    spec.expectedRows,
    spec.headers.length,
