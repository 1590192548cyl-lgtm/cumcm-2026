...
\end{equation}
结果文件的结束行不直接取理论根，而按式\eqref{eq:display_safe}增加显示与离散双重余量。正式重算得到结束时刻 $205980\unit{s}=57.2167\unit{h}$，计算最大值为 $0.1500\unit{kg/kg}$（四位显示值），满足 $C_{\max}\le0.15\unit{kg/kg}$。表\ref{tab:q3}列出关键时刻的径向分布。
