...
本文针对圆柱形中药材热风烘干过程，建立四问统一的一维轴对称热质传递模型。温度场由热传导方程描述，水分场由非线性扩散方程描述，表面均采用第三类边界条件；空间上以节点中心守恒有限体积法离散，时间上采用 BDF 法求解刚性半离散系统，并以全隐式有限体积格式作独立交叉验证。
...
\textbf{针对问题三，}将“给定时刻求分布”转化为“给定阈值求首达时刻”，以事件函数 $g(t)=C(0,t)-0.15$ 连续定位终点。两种独立离散方法给出的达标时间为 $57.16\sim57.21\unit{h}$，因此报告烘干时长为 \textbf{约 $57.2\unit{h}$（$2.38\unit{d}$）}；考虑四位小数显示误差和网格差异后，结果文件末行为 $205980\unit{s}=57.2167\unit{h}$。

\textbf{针对问题四，}令 $\xi=r/R(t)$ 为随收缩运动的无量纲物质坐标，以附件 2 的实测半径路径构造移动边界模型。两种独立离散方法给出的达标时间为 $50.82\sim50.86\unit{h}$，故报告为 \textbf{约 $50.9\unit{h}$（$2.12\unit{d}$）}，结束半径约为 \textbf{$1.200\unit{cm}$}。同物性反事实表明，缩短扩散距离是收缩加快模型内干燥的重要机制，但其幅度依赖均匀收缩假设。
...
\label{eq:drytime}
\end{equation}
数值序列中最大值始终位于中心。本文将事件函数 $g(t)=C(0,t)-0.15$ 直接交给 BDF 求解器作根定位，而不再用相邻 $60\unit{s}$ 输出点线性插值。官方文件按四位小数显示。结果文件取满足 $C_{\max}\le0.15\unit{kg/kg}$ 的首个 $60\unit{s}$ 网格点作为末行；连续事件时长用于模型解释，该网格时刻用于 Excel 最后一行。

\section{问题 1：预热阶段}

问题 1 在固定半径 $R=2\unit{cm}$ 上使用式\eqref{eq:q1_D}与常热物性。内部网格取 $N=640$、时间步 $0.1\unit{s}$，每 $1\unit{s}$ 输出一次，并插值得到每隔 $0.1\unit{cm}$ 的结果。图\ref{fig:q1profiles}显示，温度从表面向中心传播，而水分由表层先行逸出；在前 $1800\unit{s}$ 内，水分前沿尚未使中心发生可见变化。代表性节点值见表\ref{tab:q1temperature}和表\ref{tab:q1moisture}。
...
综合图\ref{fig:q1profiles}和表\ref{tab:q1temperature}、表\ref{tab:q1moisture}可知，预热 $1800\unit{s}$ 后，中心与表面温度分别为 $33.5755\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$，中心与表面水分浓度分别为 $2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$。升温已传至内部，而失水仍集中于外层，表明该阶段传热快于传质。


\section{问题 2：变物性完整烘干模型}

问题 2 用式\eqref{eq:appendix3}替换常物性，仍令 $R=2\unit{cm}$。前 $3\unit{h}$ 完全处于附件 1 的观测范围内，不涉及长期外推。正式网格取 $N=640$。图\ref{fig:q2profiles}表明，温度场在 $3\unit{h}$ 内已接近 $50\,^{\circ}\mathrm{C}$，但水分仍保留明显径向梯度，说明后续干燥时长主要受内部传质控制；详细节点值列于表\ref{tab:q2temperature}和表\ref{tab:q2moisture}。
...
问题 3 延续式\eqref{eq:appendix3}并将附件 1 的末值按式\eqref{eq:air_extension}外推。中心水分浓度随时间单调下降，且始终为截面最大值。全隐式有限体积法在 $N=640$ 时给出 $57.2108\unit{h}$；BDF 法在 $N=2560$ 时给出事件根 $57.1686\unit{h}$。两种独立时间离散的差异约为 $0.04\unit{h}$，大于单纯事件容差误差，故不将秒级数值输出等同于模型的物理精度，报告
\begin{equation}
t_{\mathrm{dry},3}\approx57.2\unit{h}\approx2.38\unit{d}.
\end{equation}
结果文件的结束行取满足判据的首个 $60\unit{s}$ 网格点，不直接用事件根外推。该点对应
\[
t_{\mathrm{end},3}=205980\unit{s}=57.2167\unit{h},
\]
此时计算最大值的四位显示结果满足
\[
C_{\max}=0.1500\unit{kg/kg}\le0.15\unit{kg/kg}.
\]
表\ref{tab:q3}列出关键时刻的径向分布。
...
综上，固定半径且采用附录 3 物性时，理论达标时间为约 $57.2\unit{h}$（$2.38\unit{d}$）。结果文件末行为 $205980\unit{s}=57.2167\unit{h}$，此时 $C_{\max}=0.1500\unit{kg/kg}$（四位显示值），全域满足 $C\le0.15\unit{kg/kg}$。


\section{问题 4：考虑半径收缩的烘干时间}

问题 4 使用式\eqref{eq:appendix4}和式\eqref{eq:moving_heat}--\eqref{eq:moving_mass}。附件 2 显示半径由 $2.000\unit{cm}$ 快速减小，并在长时段接近 $1.198\unit{cm}$。全隐式有限体积法与 $N=2560$ 的 BDF 法分别给出 $50.8634\unit{h}$ 与 $50.8243\unit{h}$，据其差异报告
\begin{equation}
t_{\mathrm{dry},4}\approx50.9\unit{h}\approx2.12\unit{d},
\end{equation}
此时半径约为 $1.200\unit{cm}$。提交末行取 $183120\unit{s}=50.8667\unit{h}$，其显示安全校验为
\begin{equation}
C_{\max}=0.1500\ \text{（四位显示值）}\le0.15.
\end{equation}
因此该末行即为判据首次满足的 $60\unit{s}$ 网格点。图\ref{fig:endpoint}直观展示两问的达标前沿与收缩边界。
...
因此，采用附录 4 物性与实测收缩路径时，理论达标时间为约 $50.9\unit{h}$（$2.12\unit{d}$），结束半径约为 $1.200\unit{cm}$。结果文件末行为 $183120\unit{s}=50.8667\unit{h}$，满足 $C_{\max}=0.1500\unit{kg/kg}$（四位显示值）。
...
\midrule
附录 3 & 57.2108 & 25.1730 & $56.00\%$ \\
附录 4 & 129.1351 & 50.8634 & $60.61\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
两套物性下，收缩路径均缩短模型预测时长，方向一致。这一结果说明缩短扩散距离是重要机制；但 $56.00\%$ 和 $60.61\%$ 的幅度依赖式\eqref{eq:moving_mass}的物质坐标假设，不能外推为任意药材的普遍比例。
...
第一，四问共享同一有限体积框架，常物性、变物性、时变边界和收缩半径只通过参数函数替换，降低了多套程序之间的口径误差。第二，阈值事件由 BDF 根函数连续定位，提交终点同时覆盖四位小数显示误差和网格差异。第三，全隐式有限体积格式与 BDF 法构成独立求解器对照，避免以单一数值实现自证。第四，本文没有将问题 3、4 的联合变化误判为收缩的单独贡献，而是使用同物性反事实分离半径路径效应。第五，输入检查、网格加密、有效 PDE 积分平衡、退化测试、灵敏度和 Excel 检查均纳入自动化流程；两份原始附件的哈希也已核对一致。
...
\textbf{问题 3：}固定半径、附录 3 物性下的理论达标时间为约 $57.2\unit{h}$；结果文件末行取 $205980\unit{s}=57.2167\unit{h}$。

\textbf{问题 4：}附录 4 物性与实测收缩路径下的理论达标时间为约 $50.9\unit{h}$，结束半径约为 $1.200\unit{cm}$；结果文件末行取 $183120\unit{s}=50.8667\unit{h}$。同物性反事实支持“收缩缩短扩散距离并加快模型内干燥”的机制判断，但具体幅度依赖均匀收缩闭合，不能脱离模型设定作普遍推广。
