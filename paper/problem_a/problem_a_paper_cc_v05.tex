...
预热 $1800\unit{s}$ 后，中心与表面温度分别为 $33.5755\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$，中心与表面水分浓度分别为 $2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$。升温已传至内部，而失水仍集中于外层，传热快于传质。
...
$3\unit{h}$ 时，中心与表面温度之差仅为 $0.1169\,^{\circ}\mathrm{C}$，而中心与表面水分浓度之差仍为 $0.7581\unit{kg/kg}$。温度场已接近平衡，水分场仍有显著径向梯度，后续干燥主要受内部扩散控制。
...
\end{equation}
结果文件的结束行取满足判据的首个 $60\unit{s}$ 网格点，不直接用事件根外推；该点为 $205980\unit{s}=57.2167\unit{h}$，计算最大值为 $0.1500\unit{kg/kg}$（四位显示值），满足 $C_{\max}\le0.15\unit{kg/kg}$。表\ref{tab:q3}列出关键时刻的径向分布。
...
固定半径且采用附录 3 物性时，理论达标时间为约 $57.2\unit{h}$（$2.38\unit{d}$）。结果文件末行为 $205980\unit{s}=57.2167\unit{h}$，此时 $C_{\max}=0.1500\unit{kg/kg}$（四位显示值），全域满足 $C\le0.15\unit{kg/kg}$。
...
采用附录 4 物性与实测收缩路径时，理论达标时间为约 $50.9\unit{h}$（$2.12\unit{d}$），结束半径约为 $1.200\unit{cm}$。结果文件末行为 $183120\unit{s}=50.8667\unit{h}$，满足 $C_{\max}=0.1500\unit{kg/kg}$（四位显示值）。
