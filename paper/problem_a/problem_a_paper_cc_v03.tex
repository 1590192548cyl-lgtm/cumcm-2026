...
\definecolor{resultblue}{HTML}{1F4E79}
\definecolor{resultfill}{HTML}{F3F7FB}
\newcommand{\resultbox}[2]{%
  \par\medskip\noindent
  \begingroup
  \setlength{\fboxsep}{7pt}%
  \fcolorbox{resultblue}{resultfill}{%
    \parbox{\dimexpr\linewidth-2\fboxsep-2\fboxrule\relax}{%
      \textcolor{resultblue}{\heiti #1}\par\smallskip
      \normalfont #2}}
  \endgroup
  \par\medskip}
...
\textbf{针对问题一，}采用附录 2 的常热物性和浓度相关扩散系数，求解预热阶段温度与水分的径向分布。结果表明，$1800\unit{s}$ 时中心与表面温度分别为 \textbf{$33.5753\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$}，相应干基含水率分别为 \textbf{$2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$}；热量已向内部传递，而失水仍主要集中在表层。
...
\textbf{针对问题三，}将“给定时刻求分布”转化为“给定阈值求首达时刻”，以事件函数 $g(t)=C(0,t)-0.15$ 连续定位终点。两种独立离散方法给出的达标时间为 $57.16\sim57.21\unit{h}$，因此报告烘干时长为 \textbf{约 $57.2\unit{h}$（$2.38\unit{d}$）}；考虑四位小数显示误差和网格差异后，结果文件取 $57.2500\unit{h}$ 作为安全结束时刻。
...
\resultbox{问题 1 核心结论}{预热 $1800\unit{s}$ 后，中心与表面温度分别为 $33.5753\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$，中心与表面水分浓度分别为 $2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$。升温已传至内部，而失水仍集中于外层，传热快于传质。}
...
\resultbox{问题 2 核心结论}{$3\unit{h}$ 时，中心与表面温度之差仅为 $0.1169\,^{\circ}\mathrm{C}$，而中心与表面水分浓度之差仍为 $0.7581\unit{kg/kg}$。温度场已接近平衡，水分场仍有显著径向梯度，后续干燥主要受内部扩散控制。}
...
\resultbox{问题 3 核心结论}{固定半径且采用附录 3 物性时，理论达标时间为约 $57.2\unit{h}$（$2.38\unit{d}$）。结果文件取 $57.2500\unit{h}$ 作为安全结束时刻，此时 $C_{\max}+\varepsilon_C=0.1499443\unit{kg/kg}<0.14995\unit{kg/kg}$，四位小数显示下仍满足全域达标要求。}
...
\resultbox{问题 4 核心结论}{采用附录 4 物性与实测收缩路径时，理论达标时间为约 $50.9\unit{h}$（$2.12\unit{d}$），结束半径约为 $1.200\unit{cm}$。结果文件取 $50.8833\unit{h}$ 作为安全结束时刻，满足 $C_{\max}+\varepsilon_C=0.1499204\unit{kg/kg}<0.14995\unit{kg/kg}$。}
...
\textbf{问题 1：}预热 $1800\unit{s}$ 后，中心与表面温度分别为 $33.5753\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$，水分浓度分别为 $2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$。温升已传至内部，而失水主要发生在表层。

\textbf{问题 2：}$3\unit{h}$ 时中心与表面温度仅相差 $0.1169\,^{\circ}\mathrm{C}$，水分浓度仍相差 $0.7581\unit{kg/kg}$。温度场先于水分场趋于均匀，后续烘干主要受内部扩散控制。

\textbf{问题 3：}固定半径、附录 3 物性下的理论达标时间为约 $57.2\unit{h}$；计入显示误差和网格差异后，安全结束时刻取 $57.2500\unit{h}$。

\textbf{问题 4：}附录 4 物性与实测收缩路径下的理论达标时间为约 $50.9\unit{h}$，结束半径约为 $1.200\unit{cm}$；安全结束时刻取 $50.8833\unit{h}$。同物性反事实支持“收缩缩短扩散距离并加快模型内干燥”的机制判断，但具体幅度依赖均匀收缩闭合，不能脱离模型设定作普遍推广。
