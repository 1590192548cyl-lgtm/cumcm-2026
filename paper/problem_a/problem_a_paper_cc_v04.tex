...
预热 $1800\unit{s}$ 后，中心与表面温度分别为 $33.5753\,^{\circ}\mathrm{C}$ 和 $36.7856\,^{\circ}\mathrm{C}$，中心与表面水分浓度分别为 $2.5500\unit{kg/kg}$ 和 $1.5102\unit{kg/kg}$。升温已传至内部，而失水仍集中于外层，传热快于传质。
...
固定半径且采用附录 3 物性时，理论达标时间为约 $57.2\unit{h}$（$2.38\unit{d}$）。结果文件取 $57.2500\unit{h}$ 作为安全结束时刻，此时 $C_{\max}+\varepsilon_C=0.1499443\unit{kg/kg}<0.14995\unit{kg/kg}$，四位小数显示下仍满足全域达标要求。
...
采用附录 4 物性与实测收缩路径时，理论达标时间为约 $50.9\unit{h}$（$2.12\unit{d}$），结束半径约为 $1.200\unit{cm}$。结果文件取 $50.8833\unit{h}$ 作为安全结束时刻，满足 $C_{\max}+\varepsilon_C=0.1499204\unit{kg/kg}<0.14995\unit{kg/kg}$。
